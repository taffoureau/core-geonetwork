...
import jeeves.utils.Util;
	...
	public class JeevesServletMultiple extends JeevesServlet
	{
	    ...
	    public void init() throws ServletException {
	        ...
	        for (String item : items ){
	        	JeevesEngine jeevesTemp = new JeevesEngine();
	        	System.out.println("ITEM = " + item);
	        	String dynConfigPath = configPath + "-" + item;
	        	dynConfigPath = dynConfigPath + File.separator;
	            System.out.println("CONFIG PATH DYNAMIC IS SET : " + dynConfigPath);
	            jeevesTemp.init(appPath, dynConfigPath, baseUrl, this, item);
	            jeevesMap.put(item, jeevesTemp);
	        }
...
//			}
			...
			for (String key : jeevesMap.keySet()) {
				...
				if (req.getServletPath().startsWith("/" + key)){
					jeevesMap.get(key).dispatch(srvReq, session);
					break;
				}
